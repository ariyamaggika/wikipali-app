...
/// <summary>
/// 文章句子特殊语法 Text Mesh Pro版
/// 术语&注释&句子
/// {{type|base64}}
/// </summary>
public class ArticleMarkdownTMPManager
{
    private ArticleMarkdownTMPManager() { }
    private static ArticleMarkdownTMPManager manager = null;
    //静态工厂方法 
    public static ArticleMarkdownTMPManager Instance()
    {
        if (manager == null)
        {
            manager = new ArticleMarkdownTMPManager();
        }
        return manager;
    }
    ...
    string PrefilterSentenceTerm(string sentence)
    {
        ...
        for (int i = 0; i < mArr.Length; i++)
        {
            ...
            string colorText = CommonTool.COLOR_DARK_YELLOW;
            if (!termDic.ContainsKey(json.meaning))
            {
                colorText = CommonTool.COLOR_DARK_YELLOW;
            }
            else
            {
                if (termDic[json.meaning].isCommunity)
                    colorText = CommonTool.COLOR_DARK_GREEN;
                else
                    colorText = CommonTool.COLOR_DARK_BLUE;
            }
            string colorFillWord = string.Format(CommonTool.RICHTEXT_COLOR_FLAG, colorText, fillWord);
            //todo color
            string linkText = string.Format(CommonTool.TPM_RICHTEXT_LINK_FLAG, fillWord, colorFillWord);
            res = front + linkText + back;
            offset += mArr[i].Value.Length - linkText.Length;//<>占两个符号
        }
        return res;
    }
    ...
    string PrefilterSentenceNote(string sentence)
    {
        ...
        for (int i = 0; i < mArr.Length; i++)
        {
            ...
            string fillWord = "[注释" + id + "]";
            string colorFillWord = string.Format(CommonTool.RICHTEXT_COLOR_FLAG, CommonTool.COLOR_DARK_BLUE2, fillWord);
            //todo color
            string linkText = string.Format(CommonTool.TPM_RICHTEXT_LINK_FLAG, fillWord, colorFillWord);
            res = front + linkText + back;
            offset += mArr[i].Value.Length - linkText.Length;//[注释]占4个符号
        }
        return res;
    }
