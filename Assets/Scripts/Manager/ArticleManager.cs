...
public class ArticleManager
{
    ...
    /// <summary>
    /// 输入BookID，返回指定pargraph上级的所有level>level的book数据
    /// </summary>
    /// <param name="input"></param>
    /// <returns></returns>
    ...
    /// <summary>
    /// 输入BookID，返回指定pargraph范围内的level>1&&<100的book数据
    /// </summary>
    /// <param name="input"></param>
    /// <returns></returns>
    public List<BookDBData> GetBookChildrenAllFromID(int bookID, int minPargraph, int maxPargraph)
    {
        ...
        dbManager.Getdb(db =>
        {
            var readerPali = db.SelectArticleChildrenAll(bookID.ToString(), minPargraph.ToString(), maxPargraph.ToString());
            Dictionary<string, object>[] paliPairs = SQLiteTools.GetValues(readerPali);
            if (paliPairs != null)
            {
                int paliLength = paliPairs.Length;
